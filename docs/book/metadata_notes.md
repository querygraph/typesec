...
and the current project version, for example `typesec (0.4.0)`. This short title
...
```xml
<dc:title id="epub-title-1">typesec (0.4.0)</dc:title>
...
`covers typesec (0.4.0)` because it is generated from the same catalog name as
the Kindle metadata.
...
as `Typesec`. The generated catalog name, such as `typesec (0.4.0)`, belongs in
Kindle-facing metadata, upload filenames, `VERSION.md`, and any explicit small
build subtitle such as `covers typesec (0.4.0)`.

If a modern Kindle shows a bare lowercase title such as `typesec` even though
the intended catalog name is `typesec (0.4.0)`, look for catalog fallbacks rather
...
`typesec (0.4.0)`, validators that use `grep -E` must escape the expected title
